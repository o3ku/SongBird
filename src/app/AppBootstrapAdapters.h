...
class FunctionRuntimeProfileResolver final : public IRuntimeProfileResolver
{
public:
    std::function<std::optional<VmessItem>()> resolveActiveServerFn;
    std::function<CoreType(const VmessItem&)> resolveLaunchCoreTypeFn;
    std::function<CoreInfo(const VmessItem&)> resolveCoreInfoFn;
    std::function<QString(const VmessItem&)> resolveRuntimeConfigPathFn;
    std::function<QStringList(CoreType)> resolveCoreCandidatesFn;
    std::function<QString(const QStringList&)> locateFirstExistingFileFn;
    std::function<QString()> currentIndexIdFn;
    std::function<CoreType(CoreType)> resolveRuntimeCoreTypeFn;
    std::function<QString(CoreType)> resolveCoreInstallDirectoryFn;
...
public:
    ...
    std::function<void()> removeStaleSingBoxCacheFn;
