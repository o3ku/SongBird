...
int toLegacyConfigTypeValue(ConfigType type)
{
    switch (type) {
...
QJsonValue toLegacyCoreTypeValue(CoreType type)
{
    switch (type) {
    case CoreType::Xray:
        return 2;
...
struct RootConfigMapper {
    static void read(const QJsonObject& root, Config& config)
    {
    ...
    static void write(QJsonObject& root, const Config& config)
    {
...
struct UiConfigMapper {
    static void read(const QJsonObject& root, Config& config)
    {
        ...
        config.mainQrPreviewVisible = uiItem.value(QStringLiteral("mainQrPreviewVisible")).toBool(false);
    ...
    static void write(QJsonObject& root, const Config& config)
    {
        ...
        uiItem.insert(
            ...
            normalizeSplitPercent(config.mainServerLogSplitPercent, 60));
        uiItem.insert(
            QStringLiteral("mainServerQrSplitPercent"),
            normalizeSplitPercent(config.mainServerQrSplitPercent, 78));
...
struct CollectionConfigMapper {
    static void read(const QJsonObject& root, Config& config)
    {
        ...
        config.routingItems = parseRoutingItems(root.value(QStringLiteral("routings")).toArray());
...
{
    Config config;
    RootConfigMapper::read(root, config);
    UiConfigMapper::read(root, config);
    CollectionConfigMapper::read(root, config);
    ensureBuiltinRoutingItems(config);
    TunConfigMapper::read(root, config);
    PolicyConfigMapper::read(root, config);
    return config;
}

void applyConfig(QJsonObject& root, const Config& config)
{
    RootConfigMapper::write(root, config);
    UiConfigMapper::write(root, config);
    CollectionConfigMapper::write(root, config);
    TunConfigMapper::write(root, config);
    PolicyConfigMapper::write(root, config);
}
